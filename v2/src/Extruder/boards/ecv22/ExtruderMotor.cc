...
volatile uint16_t ext_stepper_ticks_per_step = 0;
volatile int16_t ext_stepper_counter = 0;
...
void setStepperMode(bool mode, bool external/* = false*/) {
	...
	if (stepper_motor_mode) {
		...
		// external stepper uses setExtruderMotorRPM, but setStepperMode() calls this once, so setup here
